...
class AgendamentoAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.user_outro_paciente = User.objects.create_user(username='outro_paciente', password='testpassword')
        self.outro_paciente = Paciente.objects.create(usuario=self.user_outro_paciente, nome_completo='Outro Paciente Teste')
        ...
        self.agendamento_url = reverse('agendamento-detail', kwargs={'pk': self.agendamento.pk})
    ...
    def test_confirmar_agendamento(self):
        ...
        response = self.client.post(reverse('agendamento-confirmar', kwargs={'pk': self.agendamento.pk}))
    ...
    def test_cancelar_agendamento(self):
        ...
        self.client.force_authenticate(user=self.user_paciente)
        response = self.client.post(reverse('agendamento-cancelar', kwargs={'pk': self.agendamento.pk}), {'motivo_cancelamento': 'Imprevisto'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.agendamento.refresh_from_db()
    ...
    def test_reagendar_agendamento(self):
        ...
        response = self.client.post(reverse('agendamento-reagendar', kwargs={'pk': self.agendamento.pk}), {
