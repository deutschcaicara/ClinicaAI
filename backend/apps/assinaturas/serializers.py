...
class DocumentoSerializer(serializers.ModelSerializer):
    class Meta:
        model = Documento
        fields = '__all__'
        read_only_fields = ['uuid', 'data_criacao', 'data_atualizacao', 'hash_documento', 'versao', 'analise_automatizada']

    def validate(self, data):
        # Validação para garantir que a data de expiração não seja anterior à data atual
        ...
        if not data.get('compliance_regulamentar'):
            raise serializers.ValidationError("É necessário especificar a conformidade regulamentar para garantir que o documento atende aos padrões legais.")
        # Validação de interoperabilidade
        if data.get('associado_prontuario') and not data.get('paciente'):
            raise serializers.ValidationError("Documentos associados ao prontuário devem estar vinculados a um paciente.")
        return data

    def create(self, validated_data):
        # Lógica adicional ao criar um documento, como definir a expiração padrão e gerar o hash
...
class AssinaturaSerializer(serializers.ModelSerializer):
    class Meta:
        model = Assinatura
        fields = '__all__'
        read_only_fields = ['uuid', 'data_assinatura', 'assinatura_eletronica', 'historico_eventos']
    ...
    def validate(self, data):
        ...
        if data.get('documento') and data.get('assinante'):
            ...
            if documento.is_expired():
                raise serializers.ValidationError("Não é possível assinar um documento expirado.")
            # Validação de dupla autenticação
            if documento.consentimento_informado and not data.get('dupla_autenticacao'):
                raise serializers.ValidationError("Documentos que exigem consentimento informado precisam de autenticação em duas etapas.")
        # Verificação de integridade de assinatura com hash
        if data.get('assinatura_eletronica') and data.get('biometria_hash'):
            if not self.validar_integridade_assinatura(data['assinatura_eletronica'], data['biometria_hash']):
                raise serializers.ValidationError("A integridade da assinatura não foi confirmada.")
        return data
