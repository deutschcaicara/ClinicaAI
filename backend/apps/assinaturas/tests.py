...
class AssinaturasAPITestCase(APITestCase):
    def setUp(self):
        # Configurar usuários, paciente, profissional, documento e assinatura para os testes
        ...
        self.documento_url = reverse('documento-detail', kwargs={'pk': self.documento.pk})

    def test_assinar_documento(self):
        # Teste para assinar um documento
        self.client.force_authenticate(user=self.user_profissional)
        response = self.client.post(reverse('documento-assinar', kwargs={'pk': self.documento.pk}), {
    ...
    def test_assinar_documento_expirado(self):
        ...
        self.documento.data_expiracao = timezone.now().date() - timezone.timedelta(days=1)
        self.documento.save()
        self.client.force_authenticate(user=self.user_profissional)
        response = self.client.post(reverse('documento-assinar', kwargs={'pk': self.documento.pk}), {
            'dupla_autenticacao': True,
        ...
        self.assertIn('Não é possível assinar um documento expirado.', response.data['detail'])
    ...
    def test_cancelar_assinatura(self):
        ...
        response = self.client.post(reverse('assinatura-cancelar-assinatura', kwargs={'pk': assinatura.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_validar_assinatura(self):
        ...
        response = self.client.post(reverse('assinatura-validar-assinatura', kwargs={'pk': assinatura.pk}), {
    ...
    def test_notificacao_assinatura_documento(self, mock_send_notification):
        # Teste para garantir que a notificação é enviada ao assinar um documento
        self.client.force_authenticate(user=self.user_profissional)
        response = self.client.post(reverse('documento-assinar', kwargs={'pk': self.documento.pk}), {
            'dupla_autenticacao': True,
            'biometria_hash': 'hash_teste',
