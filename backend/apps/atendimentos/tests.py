...
class AtendimentoAPITestCase(APITestCase):
    def setUp(self):
        # Configurar usuários, paciente, profissional e atendimento para os testes
        self.client = APIClient()
        self.user_paciente = User.objects.create_user(username='paciente', password='testpassword')
        self.paciente = Paciente.objects.create(usuario=self.user_paciente, nome_completo='Paciente Teste')

        self.user_profissional = User.objects.create_user(username='profissional', password='testpassword')
        self.profissional = Profissional.objects.create(usuario=self.user_profissional, nome_completo='Profissional Teste')

        self.user_outro_profissional = User.objects.create_user(username='outro_profissional', password='testpassword')
        self.outro_profissional = Profissional.objects.create(usuario=self.user_outro_profissional, nome_completo='Outro Profissional Teste')
        ...
        self.atendimento_url = reverse('atendimento-detail', kwargs={'pk': self.atendimento.pk})
    ...
    def test_concluir_atendimento(self):
        ...
        response = self.client.post(reverse('atendimento-concluir', kwargs={'pk': self.atendimento.pk}))
    ...
    def test_concluir_atendimento_permissao_negada(self):
        # Teste para garantir que outro profissional não possa concluir o atendimento
    ...
    def test_cancelar_atendimento(self):
        ...
        self.client.force_authenticate(user=self.user_profissional)
        response = self.client.post(reverse('atendimento-cancelar', kwargs={'pk': self.atendimento.pk}), {'motivo_cancelamento': 'Imprevisto'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.atendimento.refresh_from_db()
    ...
    def test_reagendar_atendimento(self):
        ...
        response = self.client.post(reverse('atendimento-reagendar', kwargs={'pk': self.atendimento.pk}), {
    ...
    def test_notificacao_concluir_atendimento(self, mock_send_notification):
        # Teste para garantir que a notificação é enviada ao concluir o atendimento
        self.client.force_authenticate(user=self.user_profissional)
        response = self.client.post(reverse('atendimento-concluir', kwargs={'pk': self.atendimento.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
