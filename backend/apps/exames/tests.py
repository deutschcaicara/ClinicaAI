...
class ExameAPITestCase(APITestCase):
    ...
    def test_registrar_resultado_exame(self):
        ...
        response = self.client.post(reverse('exame-registrar-resultado', kwargs={'pk': self.exame.pk}), {
    ...
    def test_registrar_resultado_permissao_negada(self):
        # Teste para garantir que outro profissional não possa registrar o resultado do exame
        self.client.force_authenticate(user=self.outro_profissional)
        response = self.client.post(reverse('exame-registrar-resultado', kwargs={'pk': self.exame.pk}), {
            'resultados': 'Resultados alterados',
            'status': 'Realizado'
        })
    ...
    def test_cancelar_exame(self):
        ...
        self.client.force_authenticate(user=self.user_profissional)
        response = self.client.post(reverse('exame-cancelar-exame', kwargs={'pk': self.exame.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.exame.refresh_from_db()
    ...
    def test_registrar_resultado_sem_data_realizacao(self):
        # Teste para tentar registrar o resultado sem fornecer a data de realização
        self.client.force_authenticate(user=self.user_profissional)
        response = self.client.post(reverse('exame-registrar-resultado', kwargs={'pk': self.exame.pk}), {
            'resultados': 'Resultados normais',
            'status': 'Realizado'
    ...
    def test_notificacao_registrar_resultado(self, mock_send_notification):
        # Teste para garantir que a notificação é enviada ao registrar o resultado do exame
        self.client.force_authenticate(user=self.user_profissional)
        response = self.client.post(reverse('exame-registrar-resultado', kwargs={'pk': self.exame.pk}), {
            'resultados': 'Resultados normais',
            'status': 'Realizado',
    ...
    def test_integracao_machine_learning(self, mock_analisar_resultados):
        # Teste para garantir que a integração com Machine Learning seja acionada ao registrar resultados
        self.client.force_authenticate(user=self.user_profissional)
        response = self.client.post(reverse('exame-registrar-resultado', kwargs={'pk': self.exame.pk}), {
            'resultados': 'Resultados normais',
            'status': 'Realizado',
