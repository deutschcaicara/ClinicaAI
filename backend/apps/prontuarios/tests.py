...
class ProntuarioAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.doctor_user = User.objects.create_user('doctor', 'doctor@example.com', 'password123')
        doctor_group, created = Group.objects.get_or_create(name='Doctor')
        self.doctor_user.groups.add(doctor_group)
        self.patient_user = User.objects.create_user('patient', 'patient@example.com', 'password123')
...
class HistoricoMedicamentosAPITestCase(APITestCase):
    def setUp(self):
        ...
        # Criação de dados iniciais
        self.medicamento = HistoricoMedicamentos.objects.create(prontuario_id=1, medicamento='Paracetamol', dosagem='500mg')
    ...
    def test_update_medicamento_as_patient(self):
        ...
        response = self.client.patch(reverse('historico_medicamentos-detail', args=[self.medicamento.id]), data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class EvolucaoClinicaAPITestCase(APITestCase):
    def setUp(self):
        ...
        # Criação de dados iniciais
        self.evolucao = EvolucaoClinica.objects.create(prontuario_id=1, descricao='Paciente com sintomas de gripe')

    def test_update_evolucao_as_doctor(self):
        self.client.force_authenticate(user=self.doctor_user)
        data = {
            'descricao': 'Paciente com febre alta e tosse'
        }
        response = self.client.patch(reverse('evolucoes_clinicas-detail', args=[self.evolucao.id]), data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_update_evolucao_as_patient(self):
        self.client.force_authenticate(user=self.patient_user)
        data = {
            'descricao': 'Tentativa de atualização não permitida'
        }
        response = self.client.patch(reverse('evolucoes_clinicas-detail', args=[self.evolucao.id]), data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class DadosVitaisAPITestCase(APITestCase):
    def setUp(self):
        ...
        # Criação de dados iniciais
        self.dados_vitais = DadosVitais.objects.create(prontuario_id=1, pressao_arterial='120/80')
    ...
    def test_create_dados_vitais_as_unauthorized_user(self):
        unauthorized_user = User.objects.create_user('unauthorized', 'unauthorized@example.com', 'password123')
